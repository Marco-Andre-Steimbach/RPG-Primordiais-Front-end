...
function MasterGiveXP() {
  ...
  function handleDivide() {
    ...
    setMessage(
      `${formatInteger(total)} XP dividido entre ${characters.length} personagens.`
    )
  }
  ...
  async function handleBulkGive() {
    ...
    if (failures === 0) {
      setMessage(
        `XP distribuído para ${successIds.length} personagens com sucesso.`
      )
